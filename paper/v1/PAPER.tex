...
\begin{itemize}
  ...
  \item \textbf{An open-source reference implementation} of the architecture, a 3GPP TR~38.811-compliant LEO NTN channel simulator, baseline receivers (LMMSE, LS, Genie-aided bound), and a training harness, released to support reproducible follow-up work.
\end{itemize}
...
For the satellite domain specifically, Hou et al.~\cite{hou2025dvampnet} propose DVAMPNet, an unfolded VAMP with residual CNN attention for device activity detection and channel estimation in asynchronous grant-free satellite IoT random access. While DVAMPNet achieves 5.7\,dB NMSE gain at 30\,dB SNR, it addresses activity detection rather than the full receiver chain, assumes perfect synchronization, and operates at SNR levels far above the LEO IoT regime ($-5$ to 0\,dB). The AAU group~\cite{dui2024} developed a deep-unfolded iterative MMSE-SIC receiver for 5G-compliant coded MIMO-OFDM, but their work targets terrestrial deployments with dense DMRS pilot structures and does not address CFO tracking.
...
The architecture unfolds $T = 5$ iterations of the VAMP algorithm, each comprising six stages:
...
The computational complexity per block is dominated by the delay-Doppler FFT ($2 \times 168 \times \log_2 168 \approx 2{,}500$ real MACs per iteration) and the VAMP linear step ($O(N_{\mathrm{sc}} \times N_{\mathrm{sym}})$ per iteration). At $T=5$, the estimated MAC count is $\sim 15{,}000$ per block, exceeding the $10{,}000$ target. Reducing to $T=3$ yields $\sim 9{,}000$ MACs, meeting the budget.
...
\paragraph{Ablation benchmarks.} Eight single-field ablations evaluating BLER, BER, and NMSE at three SNR points ($-3$, 0, 5\,dB).

\noindent\textbf{Important:} All benchmark measurements reported in this paper are from \textbf{randomly initialized} model weights. BLER $\approx 1.0$ (near random guessing for QPSK) is expected. These measurements validate the infrastructure---they are not scientifically meaningful for comparing architecture quality. The central hypothesis ($\geq 2$\,dB improvement over MMSE at $-3$\,dB) requires trained model weights.
...
The estimated MAC count per block is $\sim 15{,}000$ at $T=5$ (exceeding the $10{,}000$ target) and $\sim 9{,}000$ at $T=3$ (within budget). The FFT-based delay-Doppler transform accounts for approximately $60\%$ of the MAC budget. % TODO: replace analytical estimates with profiler-based measurements.
...
\subsection{Baseline Comparison (Random Initialization)}

Table~\ref{tab:baseline} reports BLER and BER for VAMP-JCD vs.\ baselines at random initialization. As expected for untrained models, BLER $\approx 1.0$ across all receivers. The values serve as infrastructure validation---all receivers produce finite outputs with no numerical issues---but \emph{do not} indicate relative performance.

\begin{table}[h]
  \centering
  \caption{Baseline comparison at random initialization ($f_D = 1.2$\,kHz). BLER $\approx 1.0$ is expected for untrained models.}
  \label{tab:baseline}
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    SNR (dB) & VAMP-JCD BLER & LMMSE BLER & LS BLER & Genie BLER \\
    \midrule
    $-10$ & 1.0 & 1.0 & 1.0 & 1.0 \\
    $-5$  & 1.0 & 1.0 & 1.0 & 1.0 \\
    $-3$  & 1.0 & 1.0 & 1.0 & 1.0 \\
...
All ablations produce finite, numerically stable outputs across config variants. The key observation from random-init behavior is that NMSE increases with $T$ (from $0.98$ at $T=1$ to $3.83$ at $T=10$)---each additional VAMP iteration at random initialization compounds estimation errors. After training, NMSE is expected to decrease monotonically with $T$. The small CNN denoiser ($66$ params) produces lower NMSE ($1.03$) at random init than the delay-Doppler shrinkage ($2.38$) because its weights are randomly initialized to produce near-zero output.
...
\begin{itemize}
  ...
  \item \textbf{No FPGA implementation.} The INT8 quantization target and Zynq ZU3EG SWaP targets ($<5$\,W, $<600$ DSP slices, $<1$\,ms latency) are design constraints, not validated results. FPGA resource utilization, power consumption, and latency have not been measured. This requires the FINN or Vitis AI toolchain, which is outside the current pipeline.
...
The architecture, channel simulator (3GPP TR~38.811 compliant), baseline receivers, training harness, and validation suite are released as open source to provide a reproducible foundation for follow-up work.
...
\bibitem{dui2024}
AAU Group.
\newblock Deep-unfolded iterative soft-input soft-output SIC receiver for 5G coded MIMO-OFDM.
...
\textbf{Yes.} The complete validation harness is released as open source with seeded random number generation, deterministic inference, and a comprehensive pytest suite covering shapes, gradients, numerics, domain benchmarks, and ablations. All measurements are from randomly initialized weights, which is clearly stated.
...
\textbf{Yes.} The reference implementation, channel simulator, baseline receivers, training harness, and validation suite are released as open source.

\paragraph{6. Broader impacts.}
Does the paper discuss broader impacts?
\textbf{Minimal.} This is a foundational architecture and validation harness. Broader impacts depend on deployment context. The architecture is designed for satellite IoT connectivity, which has potential positive applications (environmental monitoring, disaster response) and dual-use considerations.

\paragraph{7. Compute.}
Does the paper report compute usage?
\textbf{Partially.} The validation harness runs on CPU in under 5 minutes. No GPU training compute is reported because no trained model exists. The target FPGA compute budget ($<5$\,W, $<1$\,ms per block) is stated as a design constraint, not a measured result.
