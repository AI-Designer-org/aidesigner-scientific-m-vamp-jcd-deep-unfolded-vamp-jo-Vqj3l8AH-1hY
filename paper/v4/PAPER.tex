...
\begin{itemize}
  ...
  \item \textbf{A validation harness} covering shape correctness, gradient flow (feasibility check) through $T=5$ unfolded iterations, numerical stability (including bfloat16 compatibility), CFO estimation accuracy, parameter and MAC counting, delay-Doppler sparsity characterization, and seven single-field ablations. The eighth planned ablation (quantization sensitivity) requires the FINN/Vitis AI toolchain and is not yet executable. The harness enables reproducible characterization of architectural variants.
  \item \textbf{A reference implementation} of the architecture, a 3GPP TR~38.811-compliant LEO NTN channel simulator, baseline receivers for interface verification (LMMSE, LS, Genie-aided bound), and a training harness, provided in the accompanying artifact bundle to support reproducible follow-up work.
\end{itemize}
...
A key theoretical foundation is the Vector Approximate Message Passing (VAMP) framework of Rangan et al.~\cite{rangan2017vamp,rangan2019vamp}, which provides state-evolution guarantees for right-rotationally invariant measurement matrices---a property that makes it better motivated than AMP for the structured (non-i.i.d.) pilot matrices in OFDM systems such as the NB-IoT NPUSCH. We caution, however, that these convergence guarantees assume a large-system limit; the small-system regime studied here ($N = 168$ resource elements) may exhibit non-contractive dynamics that fall outside the scope of the rigorous proof, making VAMP a reasonable design choice whose practical convergence must be verified empirically.
...
The architecture unfolds $T = 5$ iterations of the VAMP algorithm, each comprising six stages. The validation default is $T=5$; the $T=3$ variant meets the deployment MAC budget ($\sim 9{,}000$ MACs) and is the candidate for FPGA deployment (see Section~\ref{sec:params}). At each iteration, the state includes the CFO estimate $\Delta f_t$, the channel estimate $\hat{h}_t$, and VAMP auxiliary variables $r_1, u_1$.
...
The computational complexity per block is dominated by the delay-Doppler FFT ($2 \times 168 \times \log_2 168 \approx 2{,}500$ real MACs per iteration) and the VAMP linear step ($O(N_{\mathrm{sc}} \times N_{\mathrm{sym}})$ per iteration). At the validation default of $T=5$, the estimated MAC count is $\sim 15{,}000$ per block, which exceeds the $10{,}000$ deployment budget. Reducing to $T=3$ yields $\sim 9{,}000$ MACs, meeting the budget. The $T=3$ variant is thus the currently budget-compliant deployment candidate for FPGA targets, while $T=5$ provides additional headroom for performance once the MAC budget constraint is relaxed or FFT pruning is implemented. % TODO: replace analytical MAC estimates with profiler-based measurements.

\subsection{Design Space and Ablations}

The config-driven design enables single-field ablations. Seven of the eight planned ablations are executable with the current validation harness: (1) unfolded layer count ($T = 1, 2, 3, 5, 7, 10$), (2) weight tying (shared vs.\ per-layer thresholds), (3) denoiser type (delay-Doppler shrinkage, element-wise shrinkage, small CNN, MLP), (4) CFO mode (analytic-only, analytic+learned), (5) soft symbol feedback (on/off), (6) damping strategy (shared vs.\ per-layer), and (7) noise precision source (learned vs.\ fixed). Each is a single-field change to \texttt{DU\_VAMP\_JCD\_Config}. The eighth planned ablation---quantization sensitivity (INT8 vs.\ FP32)---is defined in the config but not yet executable as it requires the FINN/Vitis AI FPGA toolchain for actual quantization and is marked as future work.
...
\paragraph{Numerical tests.} Gradient flow (feasibility/smoke check---verifies nonzero gradients through all 24 parameters in a single backward pass), numerical stability at extreme SNR ($-30$ to $+30$\,dB), zero-input stability, extreme CFO ($\pm 0.5$ cycles/symbol), and bfloat16 compatibility (forward pass loss $< 10^{-3}$ relative to float32).

\paragraph{Domain benchmarks.} BLER vs.\ SNR at three Doppler spreads (600\,Hz, 1.2\,kHz, 2.4\,kHz), BLER vs.\ Doppler spread at fixed SNR ($-3$\,dB), CFO estimation accuracy vs.\ SNR (mean absolute error), channel estimation NMSE, delay-Doppler sparsity characterization (energy concentration in top bins), and interface-verification comparison against LMMSE, LS, and Genie-aided receivers. These benchmarks validate the infrastructure and produce finite outputs---they are \emph{not} used for cross-method performance comparison at this stage.

\paragraph{Ablation benchmarks.} Seven single-field ablations (of eight planned) evaluating NMSE at three SNR points ($-3$, 0, 5\,dB). Ablation 7 (quantization sensitivity) is not yet executable.
...
\begin{itemize}
  ...
  \item \textbf{Gradient flow (feasibility check):} All 24 learnable parameters receive nonzero gradients in a single-step backward pass, confirming that the computational graph through the $\mathtt{logit\_damping} \to \mathtt{sigmoid}$ path and the VAMP state update is backpropagation-compatible.
...
\subsection{Infrastructure Verification: Receiver Interface Test}
\label{sec:infra_check}

Table~\ref{tab:baseline} reports BLER measurements for VAMP-JCD and two analytic baselines (LMMSE, LS) at random initialization. These measurements serve as a \textbf{pipeline infrastructure sanity check}---they verify that all receivers produce finite, numerically stable outputs without runtime errors and that the comparison framework correctly handles batched inference across SNR points. They do \emph{not} constitute a performance comparison: with randomly initialized weights, BLER $\approx 1.0$ across all receivers and SNR points, as expected for a near-random QPSK detector. The Genie bound is omitted here because with randomly initialized detection weights it would also yield BLER $\approx 1.0$, providing no additional infrastructure signal.

\begin{table}[h]
  \centering
  \caption{Infrastructure verification: BLER measurements at random initialization ($f_D = 1.2$\,kHz). All values are from randomly initialized weights. These are not scientifically meaningful for comparing architecture quality---they confirm only that the receiver implementations produce finite outputs without runtime errors.}
  \label{tab:baseline}
  \begin{tabular}{@{}lccc@{}}
    \toprule
...
\textbf{MAC budget trade-off.} The $\sim 15{,}000$ MAC count at $T=5$ (exceeding the $10{,}000$ target) indicates that the FFT-based delay-Doppler transform is the computational bottleneck. The $T=3$ variant ($\sim 9{,}000$ MACs) meets the budget while potentially trading off detection performance---the magnitude of this trade-off remains a quantitative question requiring trained-model verification.

\textbf{CFO tracking design.} The hybrid analytic+learned CFO estimator operates with 2 parameters (scale and bias), leveraging the strong prior from pilot autocorrelation. The mean absolute error of $0.01$--$0.006$ cycles/symbol across $-5$ to $+10$\,dB SNR provides a sufficient anchor for per-iteration residual correction, even before training.

\subsection{Literature Positioning of the Design Space}

The existing deep-unfolded receiver literature defines several distinct design points that differ in task scope, parameter budget, and deployment target. DVAMPNet~\cite{hou2025dvampnet} addresses device activity detection with $>10^4$ parameters, DUI-SISO-SIC~\cite{mazumdar2024dui} targets terrestrial coded MIMO-OFDM, and Lin~\&~Shen~\cite{lin2024hybrid} provide a hybrid CFO+channel estimator for generic OFDM---each at parameter counts and task specializations that differ from the joint receiver we present. DeepSig's OmniPHY~\cite{deepsig2024} represents a contrasting point in the design space (terrestrial 5G, $>10^5$ parameters, proprietary, x86 deployment) that is neither reproducible nor comparable on a matched LEO NTN setup. The present paper reports only the parameter-budget contrast from these cited sources as literature context; a direct, reproducible empirical comparison across architectures on the LEO NTN IoT task domain requires a trained-model evaluation that is not yet available. The extreme parameter-budget constraint we adopt ($\leq 1{,}000$ parameters) is a design choice motivated by CubeSat-class FPGA deployment targets, rather than a comparative claim against any specific prior architecture. Whether the resulting trade-off---expressiveness for hardware feasibility---yields competitive detection performance at low SNR is the central hypothesis that must be validated through training.
...
\begin{itemize}
  ...
  \item \textbf{No FPGA implementation.} The INT8 quantization target and Zynq ZU3EG design constraints ($<5$\,W, $<600$ DSP slices, targeting sub-millisecond per-block latency) are targets, not validated results. FPGA resource utilization, power consumption, and latency have not been measured. This requires the FINN or Vitis AI toolchain, which is outside the current pipeline.
  ...
  \item \textbf{Quantization sensitivity unmeasured.} The eighth planned ablation (INT8 vs.\ FP32 quantization sensitivity) is not yet executable, as it requires either the FINN/Vitis AI toolchain or a simulated quantization module that has not been implemented.
\end{itemize}

\section{Conclusion and Future Work}
\label{sec:conclusion}

We have presented a reference architecture and validation harness for VAMP-JCD, a deep-unfolded joint CFO compensation, channel estimation, and data detection receiver for LEO satellite IoT systems. The architecture uses 24 learnable parameters within a physics-constrained VAMP unfolding framework, with a delay-Doppler domain shrinkage denoiser that exploits the demonstrated sparsity of the LEO NTN channel. The validation harness verifies shape correctness, gradient flow (feasibility check), numerical stability (including bfloat16), CFO estimation accuracy, and parameter/MAC budgets, and provides seven single-field ablations (of eight planned) for design-space characterization. The eighth ablation (quantization sensitivity) requires the FINN/Vitis AI toolchain and is not yet executable.

The architecture, channel simulator (3GPP TR~38.811 compliant), baseline receivers for interface verification, training harness, and validation suite are provided in the accompanying artifact bundle to establish a reproducible foundation for follow-up work.
...
\begin{enumerate}
  ...
  \item \textbf{Characterize the performance-complexity trade-off} of unfolded iterations ($T = 1$ to $10$) to identify the optimal depth for FPGA deployment, where the marginal BLER improvement per additional iteration is no longer worth the increased compute cost.
  \item \textbf{Evaluate generalization} across orbital geometries by testing on held-out elevation angles ($15^\circ$, $75^\circ$).
  \item \textbf{Profile quantization sensitivity} using simulated INT8 quantization in PyTorch as a precursor to FPGA deployment, unblocking the eighth planned ablation.
...
\textbf{Yes.} The complete validation harness is included in the accompanying artifact bundle, with seeded random number generation, deterministic inference, and a comprehensive pytest suite covering shapes, gradients, numerics, domain benchmarks, and seven of eight planned ablations (quantization sensitivity is not yet executable). All measurements are from randomly initialized weights, which is clearly stated. No public repository URL is claimed, as the artifact accompanies this submission.
...
\textbf{In the accompanying artifact bundle.} The reference implementation, channel simulator, baseline receivers for interface verification, training harness, and validation suite are included as part of this submission. No external repository link is needed; the code is self-contained in the artifact package.
